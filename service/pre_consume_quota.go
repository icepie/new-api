...
func PreConsumeQuota(c *gin.Context, preConsumedQuota int, relayInfo *relaycommon.RelayInfo) *types.NewAPIError {
	...
	if relayInfo.OrgId > 0 {
		err := model.CheckOrgQuota(relayInfo.OrgId, preConsumedQuota)
		if err != nil {
			return types.NewErrorWithStatusCode(err, types.ErrorCodeInsufficientOrgQuota, http.StatusForbidden, types.ErrOptionWithSkipRetry(), types.ErrOptionWithNoRecordErrorLog())
		}
	}
	...
	if !user.UnlimitedQuota {
		...
		// 无限额度用户，设置一个足够大的值用于后续逻辑
		userQuota = common.GetTrustQuota() + 1
		logger.LogInfo(c, fmt.Sprintf("用户 %d 为无限额度用户, 跳过用户级别额度检查", relayInfo.UserId))
	}
	...
	if user.UnlimitedQuota {
		preConsumedQuota = 0
		logger.LogInfo(c, fmt.Sprintf("用户 %d 为无限额度用户, 不需要预扣费", relayInfo.UserId))
	} else if userQuota > trustQuota {
		// 用户额度充足，判断令牌额度是否充足
